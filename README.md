...
6. При успешной доставке сервер обновляет статус сообщения (DELIVERED) и уведомляет отправителя.
...
5. При открытии чата клиент отправляет подтверждение прочтения, сервер обновляет статус сообщения (READ) и уведомляет отправителя.
